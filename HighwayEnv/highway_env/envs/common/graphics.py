...
from .action import ActionType, DiscreteMetaAction, ContinuousAction
from ...road.graphics import WorldSurface, RoadGraphics
...
class EventHandler(object):
    ...
    def handle_continuous_action_event(cls, action_type: ContinuousAction, event: pygame.event.EventType) -> None:
        ...
        # for event in pygame.event.get():
        #     if event.type == pygame.locals.MOUSEWHEEL:
        #         action[0] += event.y / 10
        ...
        # action[1] = max(-1, min(1, action[1]))
        # action[0] = max(-1, min(1, action[0]))

        # print(pygame.joystick.get_count())

        # print(cls.wheel.get_numaxes())
        # if event.type == pygame.JOYAXISMOTION:
            # print(cls.wheel.get_axis(0), (cls.wheel.get_axis(1) + 1) / 2, (cls.wheel.get_axis(2) + 1) / 2)
        # time.sleep(0.1)
        if pygame.joystick.get_count() > 0:
            cls.wheel = pygame.joystick.Joystick(0)
            cls.wheel.rumble(0, 0.7, 500)
            if event.type == pygame.JOYAXISMOTION:
                action[1] = cls.wheel.get_axis(0)
                gas = 1 - (cls.wheel.get_axis(1) + 1) / 2
                brake = -1 + (cls.wheel.get_axis(2) + 1) / 2
                # print(gas, brake, gas + brake)
                action[0] = gas + brake
        # DEFAULT CONTROLS
        else:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_a and action_type.lateral:
                    action[steering_index] = -0.7
                if event.key == pygame.K_s and action_type.longitudinal:
                    action[0] = -0.7
                if event.key == pygame.K_w and action_type.longitudinal:
                    action[0] = 0.7
